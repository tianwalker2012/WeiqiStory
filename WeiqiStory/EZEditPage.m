...
+(CCScene *) scene
{
	...
	EZEditPage *layer = [EZEditPage node];
...
- (id) initWithEpisode:(EZEpisodeVO*)epv
{
    ...
    if(self){
        ...
        EZDEBUG(@"Begin instanciate the editor");
        editor = [EZUILoader createEditor];
        EZDEBUG(@"Complete instancitate the editor");
        editor.name.text = _episode.name;
        editor.intro.text = _episode.introduction;
        
        editor.name.delegate = self;
        editor.intro.delegate = self;
        __weak EZEditPage* weakSelf = self;
        
        editor.deleteClicked = ^(){
            EZDEBUG(@"Delete get called");
            
            if(weakSelf.deletedEpisode){
                weakSelf.deletedEpisode();
            }
            [[CCDirector sharedDirector] popScene];
        };
        ...
        editor.saveClicked = ^(){
            ...
            EZDEBUG(@"Save get called, name:%@, intro:%@", editor.name.text, editor.intro.text);
            _episode.name = editor.name.text;
            _episode.introduction = editor.intro.text;
            if(weakSelf.savedEpisode){
                weakSelf.savedEpisode();
            }
            [[CCDirector sharedDirector] popScene];
        };
        
        [editor setPosition:ccp(20, 20)];
        [[CCDirector sharedDirector].view addSubview:editor];
    }
...
{
    EZDEBUG(@"Should end editing:%@", textField.text);
    return true;
}
...
{
    EZDEBUG(@"Will end edit:%@", textField.text);
