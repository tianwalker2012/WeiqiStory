...
@implementation EZUIViewWrapper
@synthesize uiItem;
...
{
    EZDEBUG(@"SetParent get called:%@", parent);
	if(parent == nil)
...
- (void) updateUIViewTransform
{
	...
	for(CCNode *p = self; p != nil; p = p.parent)
    {
        ...
        transform = CGAffineTransformTranslate(transform, p.anchorPoint.x * box.size.width, p.anchorPoint.y * box.size.height);
    ...
    EZDEBUG(@"Final transform is:%@", NSStringFromCGAffineTransform(transform));
